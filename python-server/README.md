...
This is a sample template for python-server - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name python-server --tail
```
...
```bash
aws cloudformation delete-stack --stack-name python-server
```
